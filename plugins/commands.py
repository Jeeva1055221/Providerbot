...
async def start(client, message):
                ...
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            ...
                            [
                                InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=f'https://t.me/CCMTamil') #Don't change anything without contacting me @LazyDeveloperr
                            ]
                        ]
                    )
                )
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            files_ = await get_file_details(file_id)
            files1 = files_[0]
            title = '@CCMTAMIL'.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"{'【𝐇ᴇᴀʀᴛ 𝐓ʜɪᴇꜰ】'.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            k = await msg.reply("<b>❗️❗️❗️ <u>ɪᴍᴘᴏʀᴛᴀɴᴛ</u> ❗️❗️❗️</b>\n\nThis Movie File/Video will be deleted in <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"{'【𝐇ᴇᴀʀᴛ 𝐓ʜɪᴇꜰ】'.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
